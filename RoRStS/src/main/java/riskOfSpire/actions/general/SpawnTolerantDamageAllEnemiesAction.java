...
public class SpawnTolerantDamageAllEnemiesAction extends AbstractGameAction {
    public int damage;
    private boolean firstFrame;
    private boolean isPureDamage;
    private boolean isOrbDamage;

    public SpawnTolerantDamageAllEnemiesAction(AbstractCreature source, int baseDamage, boolean isPureDamage, boolean isOrbDamage, DamageInfo.DamageType type, AttackEffect effect, boolean isFast) {
    ...
    private int[] createDamageMatrix(int baseDamage, boolean isPureDamage, boolean isOrbDamage) {
        ...
        for (int i = 0; i < retVal.length; i++) {
            ...
            if ((isOrbDamage) && ((AbstractDungeon.getMonsters().monsters.get(i)).hasPower(LockOnPower.POWER_ID))) {
    ...
    public void update() {
        ...
        this.tickDuration();
        if (this.isDone) {
            int[] damageMatrix = createDamageMatrix(this.damage, this.isPureDamage, this.isOrbDamage);
