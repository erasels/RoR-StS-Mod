...
public class WaxQuail extends StackableRelic implements OnReceivePowerRelic {
    ...
    public boolean onReceivePower(AbstractPower abstractPower, AbstractCreature abstractCreature) {
        if(abstractPower.ID.equals("Dexterity"))
        {
            AbstractDungeon.actionManager.addToBottom(new ApplyPowerAction(AbstractDungeon.player, AbstractDungeon.player, new BlurPower(AbstractDungeon.player, relicStack)));
        }
        return true;
    }
